...
class FormIzin extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('formizin_model');
    ...
    public function index()
    {
        ...
        $data["title"] = "IDO SJ | Pengajuan Form Berpergian";
        $data['userData'] = $this->formizin_model->getUserData();
        if (isset($_POST['formIzin'])) {
            $this->_submit((object) $_POST, $data['userData']);
        }

        $this->load->view('page/FormIzin.php', $data, FALSE);
    }

    private function _checkIsIdValid($formId)
    {
        $form = $this->formizin_model->getFormData($formId);
    ...
    public function print($formId = NULL)
    {
        ...
        if (!empty($formId)) {
            // Konfigurasi mPDF dengan margin yang disesuaikan untuk header/footer
            $pdf = new \Mpdf\Mpdf([
            ...
            $data["js"] = $this->load->view("include/javascript.php", NULL, TRUE);
            $data["css"] = $this->load->view("include/css.php", NULL, TRUE);
            $data["formData"] = $this->formizin_model->getFormData($formId);
            ...
            // Load konten utama
            $html = $this->load->view('print/PrintsFormIzin.php', $data, TRUE);

            // Write CSS dan HTML ke PDF
            $pdf->WriteHTML($css, 1);
            $pdf->WriteHTML($html);
            $pdf->SetTitle('Form Izin Berpergian');

            // Output PDF
            $pdf->Output('FormIzin_' . $formId . '.pdf', 'I');
        } else {
            show_404();
        }
    }
